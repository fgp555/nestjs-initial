...
export class UserService {
  ...
  async findOne(id: string): Promise<UserEntity> {
    ...
    const user = await this.userRepository
      .createQueryBuilder('user')
      .addSelect('user.password')
      .leftJoinAndSelect('user.appointmentsAsUser', 'appointmentsAsUser')
      .leftJoinAndSelect('appointmentsAsUser.user', 'user')
      .leftJoinAndSelect('appointmentsAsUser.professional', 'professional')
      .andWhere('appointmentsAsUser.status = :status', { status: 'PENDING' }) // Filtra por estado 'PENDING' en citas como paciente
      .andWhere('appointmentsAsUser.date >= :today', { today }) // Filtra por fecha desde hoy para citas como paciente

      .leftJoinAndSelect(
        'user.appointmentsAsProfessional',
        'appointmentsAsProfessional',
      )
      .leftJoinAndSelect('appointmentsAsProfessional.user', 'userProfessional')
      .leftJoinAndSelect(
  ...
  async findByIdforSeeder(userId: number): Promise<UserEntity | null> {
    return this.userRepository.findOne({
      where: { id: userId },
      relations: ['appointmentsAsUser', 'appointmentsAsProfessional'],
    });
  }
